...
- 本地文件添加与播放（单文件、文件夹导入、歌曲页拖放文件/文件夹导入）。
- 当前播放列表管理：通过弹层统一执行追加、删除单项、清空、切歌、收藏到本地歌单。
- 播放控制：播放/暂停、上一首、下一首、单曲循环、进度条拖拽与快退/快进。
- 淡入淡出：切歌淡出后淡入、暂停前淡出、恢复播放时淡入，支持配置时长。
...
- 快捷键配置保存在本地存储，重启后可恢复。
- 设置页面作为统一入口：播放设置（淡入/淡出时长）与快捷键设置子页。
...
4. renderer 初始化各业务模块（播放、歌单、下载、账号、日推、快捷键、设置页、toast）。
...
- index.html 同时承载全局固定播放器条与播放列表弹层容器，保证主页/歌曲页切换时控制条状态连续。
