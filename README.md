...
2. [已完成] 单曲下载、歌单等
3. [已完成] 登录前端完善
4. [已完成] 账号管理（主页展示当前登录用户昵称、UID、VIP、粉丝/关注、头像；登录成功后自动刷新）
5. [已完成] 下载管理（下载队列，下载状态，失败重试等）
6. 日推、收藏、网易云歌单 获取   （即将用户的歌单、收藏、日推等内容同步到播放器中，提供更便捷的访问和管理）
7. [已完成] 下载前检测避免重复下载。尤其是 按 ID 下载 和 下载并加入当前列表 两种方式可能会有重复下载的风险，需要在下载前进行检测，避免重复下载同一首歌曲。本地下载的到`%USERPROFILE%\Music\MyPlayerDownloads`下，那么用户直接加入播放列表的，可以先存放在一个另一个目录下，如``%USERPROFILE%\Music\MyPlayerDownloads\Temp``，等到用户确认加入播放列表后再移动到正式目录下，这样可以避免直接下载到正式目录下导致的重复下载问题。这样，每次下载前，都先检测这两个目录下是否已经存在同名文件，如果存在，则提示用户是否覆盖、重命名或跳过下载（当前直接默认全跳过就OK）。注意可能需要的移动、复制操作
8. [已完成] 歌词信息获取、写入 ID3 标签等 （如果网易云API提供了歌词信息，可以在下载时一并获取并写入 ID3 标签，第三方播放器也能直接显示歌词）
9. 下载的歌曲信息，直接添加到播放列表时，名称是歌曲id，而不是歌曲名称。这里需要在下载完成后，获取歌曲的元数据信息（如歌曲名称、歌手、专辑等），并更新播放列表中的显示名称。
10. [已完成] 单曲、歌单的查询，要尽可能展示完整的信息，如歌曲名称、歌手、专辑、封面等，方便用户确认下载内容。当前缺封面 ，其他细节也可以补充完善。
11. [已完成] 歌单管理，就像下载管理一样，也从歌曲页面剥离出来，单独做一个歌单管理页面。
12. [已完成] 歌单下载，当前似乎只会下载歌单中的10首歌，需要排查原因，确保能够完整下载歌单中的所有歌曲。
13. 本地存储的歌单格式与网易云服务器API歌单格式对齐，可以完全一致，方便后续做歌单同步等功能。